cross.correlations <- function(m1,m2){
    ...
    gr1 <- cbind(gr1, correlation = apply(gr1,1,function(x){ ifelse(x[2]!=0,cor(t(m1[x[1],]),t(m2[x[2],])),-10) }) )
    print("post-processing m2 -> m1")
    gr2 <- cbind(gr2, correlation = apply(gr2,1,function(x){ ifelse(x[2]!=0,cor(t(m1[x[1],]),t(m2[x[2],])),-10) }) )
